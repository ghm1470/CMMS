...
@Component({
    selector: 'app-form-for-view',
    templateUrl: './form-for-view.component.html',
    styleUrls: ['./form-for-view.component.scss']
})
export class FormForViewComponent implements OnInit, OnChanges {
...
// ===============================
    ...
    MyToolkit = Toolkit;
    image = new Image();
    formCategorySelected = new FormCategory();
    formTitle: string;
    public mask = [/\d/, /\d/, ':', /\d/, /\d/];
    ...
    ngOnInit(): void {
        ...
        // =================
        this.formService.getOneForm(this.formId).subscribe((res: Form) => {
            if (res != null) {
                this.form = res;
    ...
    selectedFormC() {
        setTimeout(() => {
            if (this.categories) {
                // this.formTitle= this.categories.tit
                if (this.categories.some(e => e.id === this.form.formCategoryId)) {
                    this.formCategorySelected = this.categories.find(e => e.id === this.form.formCategoryId);
                }
            } else {
                this.selectedFormC();
            }
