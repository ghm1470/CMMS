...
export class FormCompleteComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
  user: UserDto.Create = new UserDto.Create();

  @Input() formStatus: string;
  @Input() activityLevelId: string;
  @Input() instanceId: string;
  @Input() doSave: boolean;
  ...
  @Output() doSaveForSend = new EventEmitter<boolean>();
    ...
    setTimeout(() => {
      DataService.getWAFRepository.subscribe((res: any) => {
        if (res) {
          ...
          console.log(this.workOrderAndFormRepository, '11111111111111111');
        }
      });
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (!isNullOrUndefined(changes.doSave)) {
      if (this.doSave === true && (!this.formDataId || this.formStatus === 'pending')) {
        console.log('5282222666')
        this.register(this.actionType.ACTION_ACCEPT);
      }
    }
    if (!isNullOrUndefined(changes.formstatus)) {
      console.log('2222333333')
  ...
  console.log('this.workOrderAndFormRepository.id2=====>', this.workOrderAndFormRepository .id);
    ...
    if (errorResult) {
      ...
      this.activityService.saveFormDataAndPutInAssociatedActivityLevel({
        ...
        console.log('عملیات انجام نشد دوباره تلاش کنید')
      });
    }
  }
