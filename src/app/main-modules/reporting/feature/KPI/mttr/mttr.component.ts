...
import {MttrService} from '../../../endpoint/mttr.service';
...
export class MttrComponent implements AfterViewInit, OnInit, OnDestroy {
    constructor(private entityService: MttrService,
    ...
    mttrCalculation() {
        ...
        const dto = new MttrDTO.MttrCalculationDto();
        ...
        this.loading = true;
        this.entityService.mttrCalculation(dto).subscribe((res: MttrDTO.MttrCalculationResDto[]) => {
            this.loading = false;
            if (res) {
                this.lineChartLabels = [];
                this.lineChartData[0].data = [];
                // this.lineChartData[0].label = this.assetList.find(e => e.id === this.selectedAssetId).name;
                this.lineChartData[0].label = 'mttr';
                if (res.length > 0) {
    ...
    setChartData(res: MttrDTO.MttrCalculationResDto[]) {
        ...
        if (this.selectedRange === 'daily') {
            ...
            for (let i = 0; i < this.diffDays + 1; i++) {
                ...
                if (dJ >= this.moment.getJaliliDateFromIso(res[0].repairDate)) {
                    ...
                    for (const r of res) {
                        ...
                        const rD = this.moment.getJaliliDateFromIso(r.repairDate);
                        if (rD === dJ) {
                            const n = Math.floor(r.mttr / 60);
                            const mode = r.mttr % 60;
                            let nn;
                            if (mode < 10) {
            ...
            for (let i = 0; i < this.diffDays + 1; i++) {
                ...
                if (dJ >= this.moment.getJaliliDateFromIso(res[0].repairDate)) {
                    ...
                    for (const r of res) {
                        ...
                        const rD = this.moment.convertIsoToJDateYM(r.repairDate).toString();
                        if (rD === dJ) {
                            count += r.count;
                    ...
                    for (const r of res) {
                        ...
                        if (rD === dJ) {
                            ...
                            if (index !== -1) {
                                ...
                                // const m = ((720 * 60) - (repairDuration)) / count;
                                const m = ((repairDuration)) / count;
            ...
            for (let i = 0; i < this.diffDays + 1; i++) {
                ...
                if (dJ >= this.moment.getJaliliDateFromIso(res[0].repairDate)) {
                    ...
                    for (const r of res) {
                        ...
                        const rD = this.moment.convertIsoToJDateY(r.repairDate).toString();
                        if (rD === dJ) {
                            count += r.count;
                            repairDuration += r.repairDuration;
                            const index = lineChartLabels.findIndex(l => l === rD);
                    ...
                    for (const r of res) {
                        ...
                        if (rD === dJ) {
                            ...
                            const index = lineChartLabels.findIndex(l => l === rD);
                            if (index !== -1) {
                                // const m = ((720 * 60 * 12) - (repairDuration)) / count;
                                const m = ((repairDuration)) / count;
                                const n = Math.floor(m / 60);
                                const mode = m % 60;
                                let nn;
