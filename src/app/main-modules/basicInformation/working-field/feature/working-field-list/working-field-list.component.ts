...
@Component({
    selector: 'app-working-field-list',
    templateUrl: './working-field-list.component.html',
    styleUrls: ['./working-field-list.component.scss']
})
export class WorkingFieldListComponent implements OnInit, OnDestroy {
    roleList = new TokenRoleList();
    term: string;
    loading: boolean;
    entityList: workingField.GetPage[] = [];
    pageSize = 10;
    pageIndex = 0;
    length = -1;
    ...
    checkUsed(item: workingField.GetPage, type: string, index?) {
        ...
        this.entityService.checkUsed({id: item.id}).subscribe((res: boolean) => {
            ...
            if (res === true) {
                DefaultNotify.notifyDanger('این رسته کاری قابل ' + ntiTitle + ' نمی باشد. ', '', NotiConfig.notifyConfig);
                return;
            } else {
                if (type === 'edit') {
    ...
    chooseSelectedItemForEdit(item: workingField.GetPage) {
    ...
    chooseSelectedItemForView(item: workingField.GetPage) {
    ...
    showModalDelete(item: workingField.GetPage, i) {
