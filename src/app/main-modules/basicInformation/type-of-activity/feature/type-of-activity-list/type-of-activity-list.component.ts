...
@Component({
    selector: 'app-type-of-activity-list',
    templateUrl: './type-of-activity-list.component.html',
    styleUrls: ['./type-of-activity-list.component.scss']
})
export class TypeOfActivityListComponent implements OnInit, OnDestroy {
    roleList = new TokenRoleList();
    term: string;
    loading: boolean;
    entityList: typeOfActivityDto.GetPage[] = [];
    pageSize = 10;
    pageIndex = 0;
    length = -1;
    ...
    checkUsed(item: workingField.GetPage, type: string, index?) {
        ...
        this.entityService.checkUsed({id: item.id}).subscribe((res: boolean) => {
            ...
            if (res === true) {
                DefaultNotify.notifyDanger('این  نوع فعالیت قابل ' + ntiTitle + ' نمی باشد. ', '', NotiConfig.notifyConfig);
                return;
            } else {
                if (type === 'edit') {
    ...
    chooseSelectedItemForEdit(item: typeOfActivityDto.GetPage) {
    ...
    chooseSelectedItemForView(item: typeOfActivityDto.GetPage) {
    ...
    showModalDelete(item: typeOfActivityDto.GetPage, i) {
