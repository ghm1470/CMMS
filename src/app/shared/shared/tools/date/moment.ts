...
export class Moment {
    ...
    public static convertJaliliToIsoDateWithTime(jDate) {
        return moment(Toolkit2.Common.Fa2En(jDate), 'jYYYY/jMM/jDD - h:m:s').toISOString();
    }
    ...
    public static convertIsoToJDate2(date) {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public static convertIsoToJDateWithTime(date) {
        ...
        if (date) {
            ...
            const h = date.split('-')[2].split('T')[1].split(':')[0];
            const min = date.split('-')[2].split('T')[1].split(':')[1];
            const s = date.split('-')[2].split('T')[1].split(':')[2].split('.')[0];
            const res = this.convertDateByTimZone(y, m, d, h, min);
            // return (res.y + '/' + res.m + '/' + res.d + '  -  ' + res.h + ':' + res.min + ':' + s);
    ...
    public static convertIsoToJDate(date) {
        ...
        if (date) {
            ...
            const d = +date.split('-')[2].split('T')[0];
            const pDate = dateConvert.convertGToP(y, m, d);
            // return (pDate[0] + '/' + pDate[1] + '/' + (pDate[2] + 1));
            if (pDate[1] < 10) {
                pDate[1] = '0' + pDate[1];
            }
            if (pDate[2] < 10) {
    ...
    public static convertIsoToJDateYM(date) {
        const dateConvert = DateConvert;
        if (date) {
            const y = date.split('-')[0];
            const m = date.split('-')[1];
    ...
    public static convertIsoToJDateY(date) {
        const dateConvert = DateConvert;
        if (date) {
            const y = date.split('-')[0];
            const m = date.split('-')[1];
